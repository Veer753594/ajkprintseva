...
export default async (request) => {
  ...
  try {
    if(request.method==='GET'){
      ...
      return json(user.role==='admin'?orders:orders.filter(o=>o.customer.username===user.username));
    }
    ...
    for(let i=0;i<body.files.length;i++){
      ...
      files.push({id:fid,originalName:cleanFileName(f.name),size:bytes.length,type:f.type||'application/octet-stream',url:`/.netlify/functions/file?order=${encodeURIComponent(id)}&file=${encodeURIComponent(fid)}`,paperType:'Plain Paper',status:'Pending'});
    }
    if(!files.length) return json({error:'No usable files were received.'},400);
    const copies=Math.max(1,parseInt(body.copies||'1',10)||1);
    const order={id,createdAt:new Date().toISOString(),customer:{name:String(body.name||user.name),phone:String(body.phone||user.phone||''),username:user.username},settings:{color:body.color||'Color',copies,side:body.side||'Single Side'},files,status:'New',adminNote:''};
    const orders=await readOrders(); orders.unshift(order); await saveOrders(orders); return json({ok:true,order});
